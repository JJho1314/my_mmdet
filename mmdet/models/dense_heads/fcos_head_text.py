...
class FCOSHeadText(AnchorFreeHead):
    ...
    def __init__(self,
        ...
        self.projection = nn.Linear(256,1024)
        self.relu = nn.ReLU(inplace=True)
        self.project = nn.Conv2d(in_channels, 1024, 3, padding=1)
    ...
    def forward_single(self, x, scale, stride):
        ...
        ## region_embeddings = self.relu(self.projection(cls_feat.permute(0, 2, 3, 1).reshape(-1, 256)))
        ## region_embeddings = torch.nn.functional.normalize(region_embeddings, p=2, dim=1)
        
        for i in range(cls_feat.size(0)):
            ## region_embeddings = self.relu(self.projection(cls_feat[0].permute(1, 2, 0).reshape(-1, 256)))
            ## region_embeddings = torch.nn.functional.normalize(region_embeddings, p=2, dim=1)
            # cls_score_text = region_embeddings @ text_features.T
            # cls_score_text = cls_score_text / self.temperature
            # cls_score_text = cls_score_text.permute(1, 0).reshape(81,cls_score.size(2),cls_score.size(3))
            region_embeddings = torch.matmul(self.project(cls_feat)[0].permute(1,2,0),text_features.T).permute(2,0,1)
            region_embeddings = torch.nn.functional.normalize(region_embeddings, p=2, dim=1)
            cls_score_text = region_embeddings / self.temperature
            cls_score_text = torch.unsqueeze(cls_score_text, dim=0)
            if i == 0:
                test_score = cls_score_text
            else:
                test_score = torch.cat((test_score,cls_score_text), 0)
        
        # cls_score_text = region_embeddings @ text_features.T
        # cls_score_text = cls_score_text / self.temperature
        
        cls_score = test_score
    ...
    def forward_embedding(self,x):
        ...
        region_embedding = [
            self.relu(self.projection(cls_score.flatten(1).permute(1, 0)))
            for cls_score in x
        ]
    ...
    def loss(self,
        ...
        flatten_cls_scores = [
            cls_score.permute(0, 2, 3, 1).reshape(-1, 81)
